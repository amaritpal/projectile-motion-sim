...
public class Simulation {
    ...
    private void saveGraph() {
        if (physicsEngine != null && !isRunning) {
            ...
            showConfirm("Graph Saved", "The graph was saved successfully!");
        }
    }
        ...
        if (!savedGraphs.isEmpty()) {
            ...
            for (int i = 0; i < savedGraphs.size(); i++) {
                System.out.printf("[%d] Launch Angle: %.1f, Initial Velocity: %.1f, Gravity: %.2f%n",
                        i + 1,
                        savedGraphs.get(i).getLaunchAngle(),
                        savedGraphs.get(i).getInitialVelocity(),
                        savedGraphs.get(i).getGravity()
                );
            }
            ...
            if (selectedIndex != -1) {
                ...
                // Update visualizer with graph data
                visualiser.clearCanvas(); // Clear any existing data
                visualiser.setMaxBounds(selectedGraph.getMaxX(), selectedGraph.getMaxY());
                visualiser.addTrajectoryData(selectedGraph.getTrajectoryData());

                // Force canvas redraw to ensure the graph is displayed
