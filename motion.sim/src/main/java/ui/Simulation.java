...
public class Simulation {
    ...
    private void saveGraph() {
        if (physicsEngine != null && !isRunning) {
            GraphData graphData = new GraphData(visualiser.getTrajectoryData(), visualiser.getMaxX(), visualiser.getMaxY());
            dataLogger.saveGraph(graphData);
            showAlert("Graph Saved", "The graph was saved successfully!");
        }
    }
    ...
    private void loadGraph() {
        ...
        if (!savedGraphs.isEmpty()) {
            GraphData selectedGraph = savedGraphs.get(0); // Extend with a selection mechanism
            visualiser.loadGraph(selectedGraph);
            showAlert("Graph Loaded", "A graph was loaded successfully!");
        } else {
