...
public class Visualiser {
    ...
    private void calculateScalingFactors() {
        this.xScalingFactor = canvasWidth / maxX;
        this.yScalingFactor = canvasHeight / maxY;
        System.out.println("Scale Factor X:" + xScalingFactor + " Y: " + yScalingFactor);
    }
    ...
    private void drawTooltipTrajectory() {
        ...
        for (DataPoint point : trajectoryData) {
            ...
            drawPointWithTooltip(point, coords);
        }
    }

    private void drawPointWithTooltip(DataPoint dataPoint, double[] coords) {
        ...
        System.out.println("Hover dot added at: (" + coords[0] + ", " + coords[1] + ")");
    }
    ...
    public void toggleToolTips() {
        showToolTips = !showToolTips; // Toggle the state
        redrawCanvas(); // Redraw the canvas to apply the change
    ...
    public void saveCurrentGraph() {
        if (trajectoryData != null && !trajectoryData.isEmpty()) {
            GraphData graphData = new GraphData(new ArrayList<>(trajectoryData), maxX, maxY);
    ...
    public void loadGraph(GraphData graphData) {
        setMaxBounds(graphData.getMaxX(), graphData.getMaxY());
        addTrajectoryData(graphData.getTrajectoryData());
    }

    public List<DataPoint> getTrajectoryData() {
        DataPoint[] dataPoints = trajectoryData.toArray(new DataPoint[trajectoryData.size()]);
